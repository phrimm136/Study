...
(define (squarer a b)
  (define (process-new-value)
    (cond ((has-value? a) (set-value! b ((lambda (x) (* x x)) (get-value a)) me))
          ((has-value? b) (if (< (get-value b) 0)
                              (error "square less than 0 -- SQUARER" (get-value b))
                              (set-value! a (sqrt (get-value b)) me)))))
; logical error on if statement (Evaluate b when forgetting b)
  (define (process-forget-value)
    (forget-value! a me)
    (forget-value! b me)
    (process-new-value))
...
(define sqr-rt (make-connector))
(define sqr (make-connector))
(squarer sqr-rt sqr)
(probe "Square-root" sqr-rt)
(probe "Square" sqr)
(set-value! sqr-rt 5 'user)
(forget-value! sqr-rt 'user)
(set-value! sqr 9 'user)
(forget-value! sqr 'user)
(set-value! sqr -9 'user)