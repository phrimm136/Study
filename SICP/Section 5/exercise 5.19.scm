...
(define (make-register name)
  (let ((contents '*unassigned*)
    ...
    (define (trace-on)
      (display "--Register tracing activated: ")
      (display name)
      (display "--")
      (newline)
      (set! trace? true))
    (define (trace-off)
      (display "--Register tracing deactivated: ")
      (display name)
      (display "--")
      (newline)
      (set! trace? false))
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
            ((eq? message 'set) (lambda (value)
                                  (if trace?
                                      (begin (display "Register changing: ")
                                             (display name)
                                             (display " from ")
                                             (display contents)
                                             (display " to ")
  ...
  (let ((pc (make-register 'pc))
    ...
    (let ((the-instruction-sequence (list (list 'initialize-stack (lambda () (stack 'initialize)))
        ...
        (let ((insts (get-contents pc)))
          (cond ((null? insts) 'done)
                ((symbol? (car insts)) (set! label (car insts))
                                       (set! offset 1) ; I will not consider the case the machine breaks at label.
                                       (advance-pc pc)
                                       (check-breakpoint))
                (else (if trace?
                          (begin (display label)
      ...
      (define (trace-off)
        ...
        (newline)
        (set! trace? false))
      (define (check-breakpoint)
        (if (member (list label offset) breakpoint-table)
            (begin (display "Breaking at ")
                   (display label)
                   (display " - ")
                   (display offset)
                   (newline)
                   (set! pause execute))
            (execute)))
      (define (set-breakpoint input-label input-offset)
        (if (member (list input-label input-offset) breakpoint-table)
            (begin (display "Attempting to set duplicate breakpoint already set")
                   (newline))
            (begin (set! breakpoint-table (cons (list input-label input-offset) breakpoint-table))
                   (display "Set breakpoint at ")
                   (display input-label)
                   (display " - ")
                   (display input-offset)
                   (newline))))
      (define (cancel-breakpoint input-label input-offset table result)
        (cond ((null? table) (display "There is no breakpoint ")
                             (display input-label)
                             (display " - ")
                             (display input-offset)
                             (newline))
              ((equal? (list input-label input-offset) (car table)) (set! breakpoint-table (append result (cdr table)))
                                                                    (display "Cancel breakpoint at ")
                                                                    (display input-label)
                                                                    (display " - ")
                                                                    (display input-offset)
                                                                    (newline))
              (else (cancel-breakpoint input-label input-offset (cdr table) (append result (list (car table)))))))
      (define (cancel-all-breakpoints)
        (set! breakpoint-table '())
        (display "Cancel all breakpoints")
        (newline))
      (define (dispatch message)
        (cond ((eq? message 'start) (set-contents! pc the-instruction-sequence)
              ...
              ((eq? message 'cancel-breakpoint) (lambda (label offset) (cancel-breakpoint label offset breakpoint-table '())))
              ((eq? message 'cancel-all-breakpoints) (cancel-all-breakpoints))
              ((eq? message 'proceed) (display "Restart")
                                      (newline)
                                      (pause))
...
(define (trace-off machine)
  (machine 'trace-off))
(define (trace-reg-on machine reg)
  ((get-register machine reg) 'trace-on))
(define (trace-reg-off machine reg)
  ((get-register machine reg) 'trace-off))
...
(define (proceed-breakpoint machine)
  (machine 'proceed))
...
(define (make-trace-on inst machine pc)
  (lambda ()
    (if (not (null? inst))
        (trace-reg-on machine (cadr inst))
        (trace-on machine))
    (advance-pc pc)))
...
(define (make-trace-off inst machine pc)
  (lambda ()
    (if (not (null? inst))
        (trace-reg-off machine (cadr inst))
        (trace-off machine))
    (advance-pc pc)))
...
(set-breakpoint fib-machine 'afterfib-n-2 5)
(start fib-machine)
(get-register-contents fib-machine 'val)
(proceed-breakpoint fib-machine)
(get-register-contents fib-machine 'val)
(proceed-breakpoint fib-machine)
(get-register-contents fib-machine 'val)
(cancel-breakpoint fib-machine 'afterfib-n-2 5)
(set-register-contents! fib-machine 'n 3)
(start fib-machine)
(get-register-contents fib-machine 'val)