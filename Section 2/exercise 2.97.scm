...
(define (install-integer-package)
  ...
  (put 'red '(integer integer) reduce-integers)
...
(define (install-rational-package)
  ...
  (define (make-rat n d)
    (let ((reduced (reduce n d)))
      (let ((var (cadr reduced)) (nn (caddr reduced)) (dd (cadddr reduced)))
        (cons (make-polynomial var nn) (make-polynomial var dd)))))
  (define (add-rat x y)
...
(define (install-polynomial-package)
  ...
  (define (reduce-poly n d)
    (if (same-variable? (variable n) (variable d))
        (make-poly (variable n) (reduce-terms (term-list n) (term-list d)))
        (error "Polys not in same var -- RED-POLY" (list p1 p2))))
  ...
  (put 'red '(polynomial polynomial) (lambda (n d) (tag (reduce-poly n d))))
...
(define (reduce n d) (apply-generic 'red n d))
...
(add rf1 rf2) ; Each entry is multiplied by -1. LUL