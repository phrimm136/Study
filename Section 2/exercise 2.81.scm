  ...
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
      (cond (proc (apply proc (map contents args)))
            ((= (length args) 2) (let ((type1 (car type-tags)) (type2 (cadr type-tags))
...
(add (make-complex-from-real-imag 3 0) (make-complex-from-real-imag 2 0))
(exp (make-complex-from-real-imag 3 0) (make-complex-from-real-imag 2 0))
; Due to inexistence complex version of exp, apply-generic conducts coercion
; which invokes apply-generic with the same arguments, and it conducts coercion, again, again, ...
; This process is infinitely iterative, which never stops with excessive memory problem.

; Louis's code is partly correct.

; The apply-generic procedure above is modified version. You can get the formal one on the book.