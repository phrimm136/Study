...
(define (install-polynomial-package)
  ...
  (define (converse-poly L outer-var inner-var)
    (define (inner-converse poly outer-order)
      (define (iter terms)
        (let ((inner-order (order (first-term terms))) (inner-coeff (coeff (first-term terms))))
          (define (conversion)
            (list inner-var
                  (make-term inner-order
                             (make-polynomial outer-var (list (make-term outer-order inner-coeff))))))
          (if (empty-termlist? (rest-terms terms))
              (conversion)
              (add-poly (conversion)
                        (iter (rest-terms terms))))))
      (if (eq? (car poly) 'polynomial)
          (iter (cddr poly))
          (list inner-var
                (make-term (make-integer 0)
                           (make-polynomial outer-var (list (make-term outer-order (cdr poly))))))))
    (if (empty-termlist? (rest-terms L))
        (inner-converse (coeff (first-term L)) (order (first-term L)))
        (add-poly (inner-converse (coeff (first-term L)) (order (first-term L)))
                  (converse-poly (rest-terms L) outer-var inner-var))))
          
  (define (add-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) (add-terms (term-list p1) (term-list p2)))
        (cond ((eq? (variable p1) 'x) (add-poly p1 (converse-poly (term-list p2) (variable p2) (variable p1))))
              ((eq? (variable p2) 'x) (add-poly (converse-poly (term-list p1) (variable p1) (variable p2)) p2))
              (else (error "Variable x needed -- ADD-POLY" (list p1 p2))))))
  (define (sub-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) (sub-terms (term-list p1) (term-list p2)))
        (cond ((eq? (variable p1) 'x) (sub-poly p1 (converse-poly (term-list p2) (variable p2) (variable p1))))
              ((eq? (variable p2) 'x) (sub-poly (converse-poly (term-list p1) (variable p1) (variable p2)) p2))
              (else (error "Variable x needed -- ADD-POLY" (list p1 p2))))))
  (define (mul-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) (mul-terms (term-list p1) (term-list p2)))
        (cond ((eq? (variable p1) 'x) (mul-poly p1 (converse-poly (term-list p2) (variable p2) (variable p1))))
              ((eq? (variable p2) 'x) (mul-poly (converse-poly (term-list p1) (variable p1) (variable p2)) p2))
              (else (error "Variable x needed -- ADD-POLY" (list p1 p2))))))
  (define (div-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (make-poly (variable p1) (div-terms (term-list p1) (term-list p2)))
        (cond ((eq? (variable p1) 'x) (div-poly p1 (converse-poly (term-list p2) (variable p2) (variable p1))))
              ((eq? (variable p2) 'x) (div-poly (converse-poly (term-list p1) (variable p1) (variable p2)) p2))
              (else (error "Variable x needed -- ADD-POLY" (list p1 p2))))))
  (define (neg-poly p)
...
(define poly-1
  (make-polynomial 'x
                   (list (list 2 (make-polynomial 'y (list (list 2 (make-integer 5))
                         ...
                         (list 1 (make-polynomial 'y (list (list 2 (make-integer 2))
...
(define poly-2
  (make-polynomial 'y
                   (list (list 2 (make-polynomial 'x (list (list 2 (make-integer 5))
                                                           (list 1 (make-integer 2)))))
                         (list 1 (make-polynomial 'x (list (list 2 (make-integer 2))
                                                           (list 1 (make-integer 1)))))
                         (list 0 (make-polynomial 'x (list (list 2 (make-integer -1))
                                                           (list 1 (make-integer 2))
