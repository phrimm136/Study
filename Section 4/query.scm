...
(define (lisp-value call frame-stream)
  (stream-flatmap (lambda (frame)
                    (if (execute (instantiate call frame (lambda (v f)
                                                           (error "Unknown pat var -- LISP-VAUE" v))))
                        (singleton-stream frame)
                        the-empty-stream))
                  frame-stream))
...
(define (execute exp)
  (apply (eval (predicate exp) user-initial-environment)
         (args exp)))
(define user-initial-environment (scheme-report-environment 5)) ; For racket users; user-initial-environemnt is defined in mit-scheme.
...
(define (find-assertions pattern frame)
  (stream-flatmap (lambda (datum) (check-assertion datum pattern frame))
