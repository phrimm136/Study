...
(define (install-raise-package)
  ...
  (define (raise-dense-to-sparse p) ((get 'make-from-coeffs 'sparse) p))
...
(define (drop x)
  (let ((dropped (project x)))
    (cond ((or (eq? (type-tag x) 'polynomial) (eq? (type-tag x) 'integer)
               (eq? (type-tag x) 'sparse) (eq? (type-tag x) 'dense)) x)
...
(define (install-projection-package)
  ...
  (put 'project '(sparse) (lambda (x) x))
  (put 'project '(dense) (lambda (x) x))
...
(define (install-polynomial-package)
  ...
  (define (install-sparse-poly-package)
    ...
    (define (make-from-terms term-list) term-list)
    (define (make-from-coeffs coeffs)
      (define (convert coeffs)
        (if (null? coeffs)
            (the-empty-termlist)
            (adjoin-term (make-term (- (length coeffs) 1) (car coeffs))
                         (convert (cdr coeffs)))))
      (make-from-terms (convert coeffs)))
    ...
    (define (zero-term? L)
      (cond ((empty-termlist? L) true)
            ...
            (else (zero-term? (rest-terms L)))))

    (define (tag p) (attach-tag 'sparse p))
    (put 'add '(sparse sparse) (lambda (p1 p2) (tag (add-terms p1 p2))))
    (put 'mul '(sparse sparse) (lambda (p1 p2) (tag (mul-terms p1 p2))))
    (put 'neg '(sparse) (lambda (p) (tag (neg-term p))))
    (put 'zero '(sparse) zero-term?)
    (put 'make-from-terms 'sparse (lambda (terms) (tag (make-from-terms terms))))
    (put 'make-from-coeffs 'sparse (lambda (coeffs) (tag (make-from-coeffs coeffs))))
    'done)
  
  (define (install-dense-poly-package)
    (define (adjoin-term term term-list order)
      (cond ((=zero? (coeff term))  term-list)
            ((= order (length term-list)) (cons (coeff term) term-list))
            (else (adjoin-term term (cons (make-integer 0) term-list) order))))
    (define (the-empty-termlist) '())
    (define (first-term term-list) (car term-list))
    (define (rest-terms term-list) (cdr term-list))
    ...
    (define (make-from-terms terms)
      (define (convert highest terms)
        (cond ((= highest 0) (cons 0 nil))
              ((> highest (order (first-term terms))) (cons 0 (convert (dec highest) terms)))
              ((= highest (order (first-term terms))) (cons (coeff (first-term terms))
                                                            (convert (dec highest) (rest-terms terms))))))
      (convert (order (first-term terms)) terms))
      ...
      (cond ((empty-termlist? L1) L2)
            ...
            (else
             (let ((t1 (first-term L1)) (t2 (first-term L2)) (o1 (order L1)) (o2 (order L2)))
               (cond ((> o1 o2) (adjoin-term t1 (add-terms (rest-terms L1) L2) o1))
                     ((< o1 o2) (adjoin-term t2 (add-terms L1 (rest-terms L2)) o2))
    ...
    (define (mul-terms L1 L2)
      (if (empty-termlist? L1)
          (the-empty-termlist)
          (add-terms (mul-term-by-all-terms (first-term L1) L2 (order L1) (order L2))
                     (mul-terms (rest-terms L1) L2))))
    (define (mul-term-by-all-terms t1 L o1 o2)
      (if (empty-termlist? L)
          (the-empty-termlist)
          (let ((t2 (first-term L)))
            (adjoin-term (make-term (+ o1 o2) (mul (coeff t1) (coeff t2)))
                         (mul-term-by-all-terms t1 (rest-terms L) o1 (- o2 1))
                         (+ o1 o2)))))
    (define (neg-term L)
      (if (empty-termlist? L)
          (the-empty-termlist)
          (adjoin-term (negate (first-term L))
                       (neg-term (rest-terms L))
                       (order L))))
    ...
    (put 'add '(dense dense) (lambda (p1 p2) (tag (add-terms p1 p2))))
    (put 'mul '(dense dense) (lambda (p1 p2) (tag (mul-terms p1 p2))))
    (put 'neg '(dense) (lambda (p) (tag (neg-term p))))
    (put 'zero '(dense) zero-term?)
    (put 'make-from-terms 'dense (lambda (terms) (tag (make-from-terms terms))))
    (put 'make-from-coeffs 'dense (lambda (coeffs) (tag (make-from-coeffs coeffs))))
    'done)
  ...
  (define (make-from-terms var  term-list) (make-poly var ((get 'make-from-terms 'sparse) term-list)))
  (define (make-from-coeffs var coeffs) (make-poly var ((get 'make-from-coeffs 'dense) coeffs)))
  ...
  (put 'make-from-terms 'polynomial (lambda (var terms) (tag (make-from-terms var terms))))
  (put 'make-from-coeffs 'polynomial (lambda (var coeffs) (tag (make-from-coeffs var coeffs))))
  'done)

(define (make-polynomial-from-terms var terms) ((get 'make-from-terms 'polynomial) var terms))
(define (make-polynomial-from-coeffs var coeffs) ((get 'make-from-coeffs 'polynomial) var coeffs))
...
(define p (make-polynomial-from-coeffs 'x (list (make-integer 1) (make-integer 2) (make-integer 1))))
(define pp (make-polynomial-from-terms 'x (list (list 2 (make-integer 1))
                                     (list 1 (make-integer 2))
                                     (list 0 (make-integer 1)))))

; coerces only to sparse
(add p p)
(sub p p)
(add pp p)
(sub p pp)
(mul (negate (make-polynomial-from-coeffs 'x (list (make-integer 1) (make-integer 1))))
     (make-polynomial-from-coeffs 'x (list (make-integer 1) (make-integer 1))))